...
pub trait Target {
    ...
    fn rd_memory(&self, _start: *const u8, _size: usize, _stream: &mut dyn Stream) -> Result<usize, Errors> {
        Ok(0)
    }
...
impl Target for VirtualTarget {
    ...
    fn rd_memory(&self, start: *const u8, size: usize, stream: &mut dyn Stream) -> Result<usize, Errors> {
...
mod tests {
    use super::*;
    use crate::command::{SupportedCommands, Command};
    ...
    fn exec_packet(result: &mut Parsed, rstream: &mut BufferedStream, target: &mut VirtualTarget) -> Result<usize, Errors> {
